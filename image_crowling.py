...
def set_product_info(product_id, product_category,product_brand,product_name,product_url,product_color_name_list,product_image_src_list) :
    '''
...
# 크롤링할 홈페이지 root url
base_url = "https://www.powderroom.co.kr"+"/rankings/c3100"

# 크롤링 base_url+"/rankings/c3100"
base_html = asyncio.get_event_loop().run_until_complete(get_html_to_web(base_url))
...
for product_title_quary in product_title_quary_list :
    ...
    p = re.compile('[0-9]+')
    product_id = p.group(product_title_quary)
    product_info = set_product_info(product_id, product_category,product_brand,product_name,product_url,product_color_name_list,product_image_src_list)
    
    create_forlder(os.getcwd()+"/image/"+product_category)
    create_forlder(os.getcwd()+"/image/"+product_category+"/"+product_name+"("+product_id+")")

    for i in range(len(product_color_name_list)) :
        download_image_by_url(product_category+"/"+product_name+"("+product_id+")"+"/"+product_color_name_list[i],product_image_src_list[i+1]) 
